...
const ImportList = () => {
  ...
  const handleSearch = (values) => {
    ...
    if (values.BrandName) {
      // Sửa: bỏ .attributes
      results = results.filter(
        (t) => t?.BrandName === values.BrandName
      );
    }
    if (values.searchText) {
      results = results.filter(
  ...
  const modelCounts = Object.values(
    filteredList.reduce((acc, item) => {
      // Sửa: bỏ .attributes
      const model = item?.BrandName || "Chưa xác định";
      if (!acc[model]) {
        acc[model] = { label: model, count: 0 };
      }
      acc[model].count += 1;
  ...
  const columns = [
    ...
    {
      title: "Số lượng",
      dataIndex: "totalimport", // Sửa: bỏ ["attributes", ...]
      key: "totalimport",
      align: "center",
    },
    {
      title: "Số lượng trả NCC",
      dataIndex: "totalimportNCC", // Sửa: bỏ ["attributes", ...]
      key: "totalimportNCC",
      align: "center",
    },
    {
    ...
    <div className="importlist-container">
      ...
      >
        ...
        <Form.Item name="BrandName">
          <Select
            ...
            allowClear
          >
            {[...new Set(importlist.map((i) => i.BrandName))].map( // Sửa: bỏ .attributes
              (brand) => (
                <Select.Option key={brand} value={brand}>
                  {brand}
                </Select.Option>
        ...
        <Form.Item name="Type">
          <Select
            ...
            allowClear
          >
            {[...new Set(importlist.map((i) => i.Type))].map( // Sửa: bỏ .attributes
              (type) => (
                <Select.Option key={type} value={type}>
                  {type}
                </Select.Option>
      ...
      <Modal
        ...
        footer={[
          ...
          detailModal.record?.Status === null && // Sửa: bỏ .attributes
          account?.Purchase === true && (
            <Button
              key="return"
              type="primary"
      ...
      >
        {detailModal.record && (
          <Descriptions bordered column={2} size="small">
            ...
            <Descriptions.Item label="Số lượng">
              {detailModal.record.totalimport} {/* Sửa: bỏ .attributes */}
            </Descriptions.Item>
            <Descriptions.Item label="Loại thiết bị">
              <Tag color="blue">
                {detailModal.record.Type || "Chưa xác định"} {/* Sửa: bỏ .attributes */}
              </Tag>
            </Descriptions.Item>
            <Descriptions.Item label="Số lượng trả NCC">
              {detailModal.record.totalimportNCC} {/* Sửa: bỏ .attributes */}
            </Descriptions.Item>
            <Descriptions.Item label="Trạng thái">
              <Tag
                color={
                  detailModal.record.Status === "Trả NCC" // Sửa: bỏ .attributes
                    ? "red"
                    : "orange"
                }
              >
