...
const SupplierList = () => {
    ...
    useEffect(() => {
        const loadSuppliers = async () => {
            try {
                ...
                const data = Array.isArray(suppliersData) ? suppliersData : (suppliersData.data || []);
                setSuppliers(data);
                setLoading(false);
            } catch (error) {
                setError(error);
    ...
    const handleExport = () => {
        const ws = XLSX.utils.json_to_sheet(
            suppliers.map(supplier => ({
                // Sửa: bỏ .attributes
                "Tên NCC": supplier.NameNCC,
                "Số điện thoại": supplier.Phone,
                "Email": supplier.Email,
                "Sản phẩm": supplier.Product,
                "Người liên hệ": supplier.NameContact
            }))
        );
        const wb = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(wb, ws, "Suppliers");
    ...
    const filteredSuppliers = suppliers.filter(supplier => {
        // Sửa: bỏ .attributes
        const name = supplier.NameNCC?.toLowerCase().trim() || "";
        const search = searchTerm.trim().toLowerCase();
        return name.includes(search);
    });
            ...
            <div className="split-container-sup">
                <div className="panel-list-sup">
                    ...
                    <ul>
                        {filteredSuppliers.map(supplier => (
                            ...
                            >
                                {/* Sửa: bỏ .attributes */}
                                <FaBuilding className="supplier-icon" /> {supplier.NameNCC}
                            </li>
                        ))}
                    </ul>
                </div>
                ...
                <div className="panel-details-sup">
                    ...
                    {selectedSupplier ? (
                        <div className="supplier-details">
                            {/* Sửa: bỏ .attributes */}
                            <p><strong>Nhà cung cấp:</strong> {selectedSupplier.NameNCC}</p>
                            <p><strong>Số điện thoại:</strong> {selectedSupplier.Phone}</p>
                            <p><strong>Email:</strong> {selectedSupplier.Email}</p>
                            <p><strong>Sản phẩm:</strong> {selectedSupplier.Product}</p>
                            <p><strong>Người liên hệ:</strong> {selectedSupplier.NameContact}</p>
                            <button onClick={handleUpdateSupplier} className="btn btn-warning">
                                <FaEdit /> Cập nhật nhà cung cấp
                            </button>
                        </div>
