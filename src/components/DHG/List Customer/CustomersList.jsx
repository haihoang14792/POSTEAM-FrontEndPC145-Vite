...
import ModalStore from './ModalStore'; // Import component đã tách
...
const CustomersList = () => {
  ...
  // Chỉ giữ lại 1 state điều khiển Modal
  ...
  const handleExport = async () => {
    try {
      setLoading(true);
      notification.info({ message: 'Đang tải dữ liệu toàn bộ cửa hàng...' });
      const res = await fetchAllCustomersForExport(searchParams);
      const rawData = res?.data || [];
      ...
      const exportData = rawData.map((item, index) => {
        ...
        return {
          ...
          'Địa chỉ Giao Hàng': data.AddressOFF || '',
          'Trạng thái': data.Status ? 'Mở' : 'Đóng',
          'Ngày mở': data.Open || '',
          'Khu vực': data.Area || ''
        };
      });
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'DanhSachCuaHang');
      XLSX.writeFile(wb, `DS_CuaHang_${new Date().toISOString().slice(0, 10)}.xlsx`);

      notification.success({ message: `Đã xuất thành công ${exportData.length} cửa hàng` });
    } catch (e) {
      console.error(e);
  ...
  const columns = [
    ...
    { title: 'Khách hàng', dataIndex: 'Customer', width: 120 },
    {
      title: 'Mã CH',
      dataIndex: 'StoreID',
      width: 100,
      ...
      <Card className="filter-card" bordered={false}>
        <Form form={form} layout="vertical" onFinish={handleSearch}>
          <Row gutter={[12, 12]} align="bottom">
            ...
            <Col xs={24} sm={24} md={8} style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
