...
import {
  ...
  deleteTicketById,
  fetchDeviceDetailHandoverPOS,
  fetchDeviceDetailRetrievePOS,
  fetchHandoverByModel,
  fetchRetrieveByModel,
} from "../../../../services/storeServices";
import {
  Button,
  Table,
...
const DeviceManagers = () => {
  ...
  // Cập nhật hàm tìm kiếm theo Serial để tìm ở cả Bàn giao và Thu hồi
  const searchBySerial = async (serial) => {
    try {
      // Gọi song song cả 2 API bàn giao và thu hồi (giả sử bạn đã import fetchDeviceDetailRetrievePOS từ storeServices)
      const [handoverRes, retrieveRes] = await Promise.all([
        fetchDeviceDetailHandoverPOS(serial),
        // Bạn cần đảm bảo đã import service này từ storeServices
        fetchDeviceDetailRetrievePOS(serial).catch(() => ({ data: [] }))
      ]);

      const handoverData = handoverRes?.data || [];
      const retrieveData = retrieveRes?.data || [];

      return [...handoverData, ...retrieveData];
    } catch (error) {
      console.error("Lỗi tìm kiếm serial:", error);
      return [];
    }
  };

  const searchByModel = async (modelName) => {
    try {
      const [handoverRes, retrieveRes] = await Promise.all([
        fetchHandoverByModel(modelName),
        fetchRetrieveByModel(modelName).catch(() => ({ data: [] }))
      ]);
      ...
      console.error("Lỗi tìm kiếm theo Model:", error);
      return [];
    }
  };
  ...
  const handleSearch = async (values) => {
    ...
    // 1. Lấy toàn bộ danh sách gốc
    let finalResults = [...tickets];

    // 2. Xử lý tìm kiếm liên quan đến thiết bị (Async) TRƯỚC
    let deviceMatchVotes = new Set();
    let hasDeviceSearch = false;

    if (values.serialNumber || values.model) {
      hasDeviceSearch = true;
      const [serialDetails, modelDetails] = await Promise.all([
        values.serialNumber ? searchBySerial(values.serialNumber) : Promise.resolve([]),
        values.model ? searchByModel(values.model) : Promise.resolve([])
      ]);

      // Gộp tất cả mã phiếu (Votes) tìm được từ thiết bị vào Set
      serialDetails.forEach(d => deviceMatchVotes.add(d.Votes));
      modelDetails.forEach(d => deviceMatchVotes.add(d.Votes));
    }

    // 3. Thực hiện lọc tổng hợp (Combined Filtering)
    finalResults = finalResults.filter((t) => {
      // Lọc theo trạng thái
      const matchStatus = values.status ? t.Status === values.status : true;

      // Lọc theo khách hàng
      const matchCustomer = values.Customer ? t.Customer === values.Customer : true;

      // Lọc theo cửa hàng
      const matchStore = values.Store ? t.Store === values.Store : true;

      // Lọc theo Mã phiếu hoặc Ticket (Số phiếu) - SỬA LỖI Ở ĐÂY
      const lowerSearch = values.searchText?.toLowerCase();
      const matchText = lowerSearch
        ? (t.Votes?.toLowerCase().includes(lowerSearch) || t.Ticket?.toLowerCase().includes(lowerSearch))
        : true;

      // Lọc theo kết quả tìm kiếm thiết bị từ API phía trên
      const matchDevice = hasDeviceSearch ? deviceMatchVotes.has(t.Votes) : true;

      // Phải thỏa mãn TẤT CẢ các điều kiện
      return matchStatus && matchCustomer && matchStore && matchText && matchDevice;
    });
      ...
      <Card bordered={false} className="shadow-sm body-card">
        ...
        <Form form={form} layout="vertical" onFinish={handleSearch} className="filter-form">
          <Row gutter={[16, 16]}>
            ...
            <Col xs={24} sm={12} md={6} lg={4}>
              <Form.Item name="model" label="Model thiết bị">
                <Input
                  prefix={<ShopOutlined className="text-muted" />}
                  placeholder="Ví dụ: T2, L1510..."
                  allowClear
                />
              </Form.Item>
            </Col>
