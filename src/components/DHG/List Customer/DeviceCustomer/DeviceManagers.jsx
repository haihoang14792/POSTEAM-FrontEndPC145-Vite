...
const DeviceManagers = () => {
  ...
  const loadTickets = async () => {
    ...
    try {
      const response = await fetchTicket();
      // Strapi v5: response.data thường là mảng trực tiếp hoặc nằm trong key data
      const ticketsArray = Array.isArray(response.data)
        ? response.data
        : response;

      if (!ticketsArray || !Array.isArray(ticketsArray)) {
        // Có thể API trả về { data: [], meta: {} } nên check kỹ
      ...
      const sortedTickets = finalArray.sort(
        (a, b) =>
          new Date(b.createdAt) - new Date(a.createdAt) // Sửa: bỏ .attributes
      );
      setTickets(sortedTickets);
    } catch (error) {
      console.error(error);
      // message.error("Lỗi khi tải danh sách phiếu!"); // Tạm ẩn để đỡ spam lỗi nếu API đang config
    }
    setLoading(false);
  };
  ...
  const fetchDevices = async () => {
    try {
      const devicesData = await fetchDevicemanager();
      // Kiểm tra cấu trúc trả về của fetchDevicemanager, giả sử nó đã trả về mảng phẳng
      const data = Array.isArray(devicesData) ? devicesData : (devicesData.data || []);

      setDevices(data);
      const options = data.map((device) => ({
        value: device.SerialNumber, // Sửa: bỏ .attributes
        label: device.SerialNumber, // Sửa: bỏ .attributes
        ...device,                  // Sửa: bỏ .attributes
      }));
      setSerialNumberOptions(options);
    } catch (error) {
      message.error("Lỗi khi tải danh sách thiết bị!");
  ...
  const canDelete =
    account?.Leader === true &&
    selectedTickets.every(
      (ticket) => ticket?.Status === "Đang tạo phiếu" // Sửa: bỏ .attributes
    );
  ...
  const searchBySerial = async (serial) => {
    try {
      ...
      if (handoverRes.length > 0) {
        // Sửa: bỏ .attributes, map trực tiếp item
  ...
  const handleSearch = async (values) => {
    ...
    if (values.searchText) {
      results = results.filter(
        (t) =>
          // Sửa: bỏ .attributes
          t?.Votes?.toLowerCase().includes(values.searchText.toLowerCase()) ||
          t?.TenderName?.toLowerCase().includes(values.searchText.toLowerCase())
      );
    }
  ...
  const getDisplayedTickets = () => {
    ...
    // Kiểm tra t tồn tại (không cần check t.attributes nữa)
  ...
  // Sửa: bỏ .attributes
  const customerList = [
    ...new Set(filteredTickets.map((t) => t.Customer)),
  ];
  // Sửa: bỏ .attributes
  const storeList = [
    ...new Set(filteredTickets.map((t) => t.Store)),
  ];
        ...
        <>
          <Table
            ...
            columns={[
              {
                title: "Số Phiếu",
                dataIndex: "Votes", // Sửa: bỏ attributes, dùng trực tiếp key
                key: "Votes",
              },
              {
                title: "Ticket",
      ...
      {isMobile && (
        <div className="mobile-ticket-list" style={{ marginTop: 20 }}>
          {paginatedTickets.map((ticket) => (
            ...
            >
              <div className="ticket-field">
                {/* Sửa: bỏ .attributes trong Mobile View */}
                <span>Mã Phiếu:</span> {ticket.Ticket}
              </div>
              <div className="ticket-field">
                <span>Khách Hàng:</span> {ticket.Customer}
              </div>
              <div className="ticket-field">
                <span>Cửa Hàng:</span> {ticket.Store}
              </div>
              <div className="ticket-field">
                <span>Trạng Thái:</span> {ticket.Status}
              </div>
            </div>
          ))}
