        ...
        if (isOpen && ticket) {
            const loadCustomer = async () => {
                try {
                    ...
                    // Strapi v5: mảng dữ liệu nằm trực tiếp hoặc trong .data
                    const customers = Array.isArray(res) ? res : (res.data || []);
                    const found = customers.find(
                        (c) =>
                            // Sửa: bỏ .attributes
                            c.Customer === ticket?.Customer &&
                            c.StoreID === ticket?.Store
                    );
                    setSelectedCustomer(found || null); // Sửa: found chính là object cần lấy, không cần .attributes
                } catch (err) {
        ...
        >
            ...
            >
                ...
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 10 }}>
                    <p style={{ width: "50%" }}>
                        ...
                        <span style={{ fontWeight: "bold", fontSize: "18px" }}>
                            {ticket?.Customer} {/* Sửa: bỏ .attributes */}
                        </span>
                    </p>
                    <p style={{ width: "50%" }}>
                        <strong>Số Phiếu:</strong>{" "}
                        <span style={{ fontWeight: "bold", fontSize: "18px" }}>
                            {ticket?.Votes} {/* Sửa: bỏ .attributes */}
                        </span>
                    </p>
                </div>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 10 }}>
                    ...
                    <p style={{ width: "50%" }}>
                        ...
                        <span style={{ fontWeight: "bold", fontSize: "18px" }}>
                            {ticket?.Person} {/* Sửa: bỏ .attributes */}
                        </span>
                    </p>
                </div>
                ...
                <h3>
                    ...
                    {selectedCustomer?.CompanyName} {/* Sửa: đã xử lý ở trên */}
                </h3>
                <p>
                    <strong>Địa chỉ / Address:</strong>{" "}
                    {selectedCustomer?.AddressOFF} {/* Sửa: đã xử lý ở trên */}
                </p>
                <p>
                    <strong>Địa chỉ nhận hàng / Address:</strong>{" "}
                    {selectedCustomer?.Address} {/* Sửa: đã xử lý ở trên */}
                </p>
                        ...
                        <table>
                            <thead>
                                <tr>
                                    ...
                                    {ticket?.Customer === "Family Mart" && ( // Sửa: bỏ .attributes
