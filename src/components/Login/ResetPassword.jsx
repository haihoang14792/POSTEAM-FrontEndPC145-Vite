...
import "./Login.scss"; // Dùng chung SCSS với Login để giữ giao diện đồng bộ
...
const ResetPassword = () => {
  ...
  const handleReset = async (e) => {
    ...
    if (!token) {
      toast.error("Thiếu mã xác thực, vui lòng kiểm tra lại link email.");
      return;
    }
    ...
    try {
      ...
      if (res?.EC === 0) {
        ...
        toast.error(res?.EM || "Link đã hết hạn hoặc không hợp lệ.");
      }
    } catch (err) {
    ...
    <div className="modern-login-page">
      <div className="login-glass-card">
        <div className="login-header">
          <h1 className="brand-title">Mật khẩu mới</h1>
          <p className="welcome-text">
            Thiết lập mật khẩu mới cho tài khoản của bạn
          </p>
        </div>

        <CForm className="modern-form" onSubmit={handleReset}>
          ...
          <div className="input-wrapper">
            ...
            <CFormInput
              type="password"
              className={`apple-input ${errors.confirm ? "error" : ""}`}
          ...
          <CButton type="submit" className="btn-modern-submit" disabled={isLoading}>
            {isLoading ? <CSpinner size="sm" /> : "Cập nhật mật khẩu"}
          </CButton>
        </CForm>
